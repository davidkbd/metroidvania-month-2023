...
func _vanish_tile(coords : Vector2i, data : TileData, rid : RID) -> void:
	print("Source id ", coords, "  ", get_cell_source_id(LAYER, coords))
	await create_tween().tween_interval(vanish_delay).finished

	set_cell(LAYER, coords, get_cell_source_id(LAYER, coords), CELL_VANISH_ANIM_TO_HIDE)
	await create_tween().tween_interval(vanish_anim_to_hide_time).finished

	set_cell(LAYER, coords, get_cell_source_id(LAYER, coords), CELL_VANISH_HIDDEN)
	await create_tween().tween_interval(vanish_hidden_time).finished

	set_cell(LAYER, coords, get_cell_source_id(LAYER, coords), CELL_VANISH_ANIM_TO_SHOW)
	await create_tween().tween_interval(vanish_anim_to_show_time).finished

	set_cell(LAYER, coords, get_cell_source_id(LAYER, coords), CELL_VANISH_NORMAL)
	_unlock_tile(rid)
